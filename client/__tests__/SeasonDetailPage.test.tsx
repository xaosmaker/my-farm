...
describe("Season Detail Page Tests", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("generates correct metadata", async () => {
    const { generateMetadata } = await import("@/app/[locale]/(protected)/seasons/[seasonId]/page");
    const metadata = await generateMetadata();
    expect(metadata.title).toBe("Seasons");
  });

  it("calls getAuth to verify user is authenticated", async () => {
    const { default: SeasonPage } = await import("@/app/[locale]/(protected)/seasons/[seasonId]/page");
    (getAuth as Mock).mockResolvedValue({
      user: { id: 1, intl: "Europe/Athens" },
    });
    (getSeason as Mock).mockResolvedValue(null);

    await SeasonPage({ params: Promise.resolve({ seasonId: "1" }) });

    expect(getAuth).toHaveBeenCalled();
  });

  it("fetches season data by ID with timezone", async () => {
    const { default: SeasonPage } = await import("@/app/[locale]/(protected)/seasons/[seasonId]/page");
    (getAuth as Mock).mockResolvedValue({
      user: { id: 1, intl: "Europe/Athens" },
    });
