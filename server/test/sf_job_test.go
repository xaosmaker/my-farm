...
func TestJob(t *testing.T) {
	...
	cases := []struct {
		...
		{"Create Jobs before season start should fail", "POST", "/api/jobs", cookie, `{"fieldId":1,"seasonId":3,"areaInMeters": 22,"jobDate":"2024-11-12T11:11:11Z","jobType":"spraying","jobSupplies":[{"quantity": 200,"supplyId": 3},{"quantity": 0.2,"supplyId": 1}]}`, 400, []string{`{"errors":[{"message":"Cannot add job before the season start","appCode":"invalid_job_start","meta":{"max":"2025-12-03 22:00:00 +0000 UTC"}}]}`}},
		{"Create Jobs should succeed", "POST", "/api/jobs", cookie, `{"fieldId":1,"seasonId":3,"areaInMeters": 22000,"jobDate":"2025-12-03T23:11:11Z","jobType":"spraying","jobSupplies":[{"quantity": 200,"supplyId": 3},{"quantity": 0.2,"supplyId": 1}]}`, 201, []string{`{"ID":6,"JobType":"spraying","Description":null,"JobDate":"`, `"AreaInMeters":22000,"Boundary":null,"SeasonID":3,"CreatedAt":"`, `"UpdatedAt":"`, `"DeletedAt":null}`}}, {"Create Jobs without field should fail", "POST", "/api/jobs", noFieldUser, `{"fieldId":1,"seasonId":1,"areaInMeters": 22,"jobDate":"2025-05-11T23:11:11Z","jobType":"spraying","jobSupplies":[{"quantity": 200,"supplyId": 3},{"quantity": 0.2,"supplyId": 1}]}`, 404, []string{`{"errors":[{"message":"Field not found","appCode":"not_found_error","meta":{"name":"Field"}}]}`}},
		{"Create Jobs in finished seadon should fail", "POST", "/api/jobs", cookie, `{"fieldId":1,"seasonId":1,"areaInMeters": 22,"jobDate":"2025-12-03T23:11:11Z","jobType":"spraying","jobSupplies":[{"quantity": 200,"supplyId": 3},{"quantity": 0.2,"supplyId": 1}]}`, 400, []string{`{"errors":[{"message":"Cannot Edit season when a season is finished","appCode":"season_finish_error","meta":null}]}`}},
		// delete

		{"delete job should fail", "DELETE", "/api/jobs/1", noFieldUser, ``, 404, []string{`{"errors":[{"message":"Job Not Found","appCode":"not_found_error","meta":{"name":"Job"}}]}`}},
		{"delete job finish season should fail", "DELETE", "/api/jobs/1", cookie, ``, 400, []string{`{"errors":[{"message":"Cannot Edit season when a season is finished","appCode":"season_finish_error","meta":null}]}`}},
		{"delete job finish season should fail", "DELETE", "/api/jobs/6", cookie, ``, 204, []string{``}},
		{"delete job with sting param should fail", "DELETE", "/api/jobs/sdfasd", cookie, ``, 400, []string{`{"errors":[{"message":"Invalid url param expect number","appCode":"invalid_url_param","meta":null}]}`}},
	}
