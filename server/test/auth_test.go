...
func TestRegister(t *testing.T) {
	cases := []struct {
		...
		{"Register with invalid password should fail", `{"email":"test@test.com","password":"test","confirmPassword":"test2"}`, 400, []string{`{"errors":[{"message":"password: Password should contains Capital letters, digits and has length greater than 8","appCode":"invalid_password","meta":{"min":"8"}}`}},
		...
		{"Register with confirm password mismatch should fail", `{"email":"test@test.com","password":"Test1srs","confirmPassword":"test2"}`, 400, []string{`{"errors":[{"message":"confirmPassword: ConfirmPassword mismatch Password","appCode":"password_mismatch_error","meta":null}]}`}},
		{"Register with exist email should fail", `{"email":"test@test.com","password":"Test1srs","confirmPassword":"Test1srs"}`, 400, []string{`{"errors":[{"message":"invalid email alread exists","appCode":"email_exist_error","meta":null}]}`}},
	...
	for _, c := range cases {
		...
		t.Run(c.name, func(t *testing.T) {
			...
			//TODO: fix this test and fail cause of mailpit
