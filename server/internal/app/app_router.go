...
func MainRouter(con *pgxpool.Pool) *chi.Mux {
	...
	r.Post("/api/users/login", httpx.Handler(authQ.LoginUser))
	r.Post("/api/users/create", httpx.Handler(authQ.CreateUser))
	r.Post("/api/users/verify", httpx.Handler(authQ.VerifyUser))
	r.Post("/api/users/resendverify", httpx.Handler(authQ.ResendVefifyEmail))
